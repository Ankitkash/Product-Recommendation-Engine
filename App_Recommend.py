...
def search_product(product_name):
    ...
    match, score, idx = process.extractOne(product_name, titles)
    
    # If match is weak, return empty
    if score < 60:
        return pd.DataFrame()
...
def ensemble_recommend(product_name):
    ...
    if matches.empty:
        return popularity_recommend(), None
    ...
    return results[:5], matches.iloc[0]['title']
...
if st.button("Recommend"):
    ...
    results, matched_product = ensemble_recommend(product)
    
    # Show matched product (IMPORTANT FIX)
    if matched_product:
        st.success(f"Showing results for: {matched_product}")
    else:
        st.warning("No exact match found. Showing popular products.")
